...
import {
  MediaCommentType,
  MediaCommentReplyType,
  MediaCommentReplyReplyType,
} from "../../handler";
import { ReactionModalHeader, ReactionModalReactors } from "../../types";

export const mediaPreparer = {
  prepareComments: (comments: MediaCommentType) => {
    const rowsPerPage = 7;
    return (
      comments &&
...
export const commentPreparer = {
  ...
  prepareReplies: (replies: MediaCommentReplyType) => {
    return (
      replies &&
      replies.map((reply) => {
        return {
...
export const replyPreparer = {
  ...
  prepareReplies: (replies: MediaCommentReplyReplyType) => {
    return (
      replies &&
      replies.map((reply) => {
        return {

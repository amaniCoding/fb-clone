...
export const getMediaInfo = async (postId: string, mediaId: string) => {
  const media = prisma.oUserPost.findUnique({
    where: {
      id: postId,
    },
    select: {
  ...
  const medias = prisma.oUserPost.findUnique({
    where: {
      id: postId,
    },
    select: {
...
export type PostMediaType = typeof updatedMedia;
export type PostMediasType = typeof updatedMedias;

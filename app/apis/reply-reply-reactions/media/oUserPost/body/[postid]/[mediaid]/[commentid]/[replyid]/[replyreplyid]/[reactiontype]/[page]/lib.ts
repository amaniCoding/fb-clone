...
export const getReactors = async (
  ...
  const updatedReactors =
    _post?.medias[0].comments[0].replies[0].replies[0].reactions.map(
      (reactor) => {
        return {
          ...reactor,
          postId: _post.id,
          ...
          replyReplyId: _post.medias[0].comments[0].replies[0].replies[0].id,
          postType: "oUserPost",
        };
      }
    );
  ...
  return {
    ...
    count: _count?.medias[0].comments[0].replies[0].replies[0]._count.reactions,
  };
};
